...
import { BookOpen, Trash2, CreditCard as Edit2, Check, Plus, X, FolderPlus } from 'lucide-react';
import type { StoryCollection } from '../../types';

interface StoryCollectionManagerProps {
  collections: StoryCollection[];
  collectionStoryCounts: Record<string, number>;
  collectionCoverImages: Record<string, string>;
  selectedCollection: StoryCollection | null;
  onSelectCollection: (collection: StoryCollection | null) => void;
  onUpdate: () => void;
  isDraggingStory?: boolean;
  onDropStory?: (collectionId: string, storyIds: string[]) => void;
}

export function StoryCollectionManager({
  collections,
  collectionStoryCounts,
  ...
  const handleDragOver = (e: React.DragEvent, collectionId: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (e.dataTransfer.types.includes('application/x-story-ids')) {
  ...
  const handleDrop = (e: React.DragEvent, collectionId: string) => {
    ...
    const storyIdsJson = e.dataTransfer.getData('application/x-story-ids');
    if (storyIdsJson && onDropStory) {
      try {
        const storyIds = JSON.parse(storyIdsJson) as string[];
        onDropStory(collectionId, storyIds);
      } catch {
        showError('Failed to add stories to collection');
      }
    }
  };
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    const { error } = await supabase.from('story_collections').insert({
      name: form.name.trim(),
  ...
  const startInlineEdit = (e: React.MouseEvent, collection: StoryCollection) => {
    e.stopPropagation();
  ...
  const handleDelete = async (id: string) => {
    ...
    setLoading(true);
    await supabase.from('story_collection_items').delete().eq('collection_id', id);
    const { error } = await supabase.from('story_collections').delete().eq('id', id);
  ...
  const startEdit = (e: React.MouseEvent, collection: StoryCollection) => {
    e.stopPropagation();
    setForm({
  ...
  const handleCollectionClick = (collection: StoryCollection) => {
  ...
  const canManage = (collection: StoryCollection) =>
    collection.created_by === user?.id || profile?.is_admin;

  const sortedCollections = useMemo(() => {
    return [...collections].sort((a, b) => {
      const countA = collectionStoryCounts[a.id] || 0;
      const countB = collectionStoryCounts[b.id] || 0;
      if (countB !== countA) return countB - countA;
      return new Date(b.created_at).getTime() - new Date(a.created_at).getTime();
    });
  }, [collections, collectionStoryCounts]);

  return (
    <div className="mb-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-medium text-[var(--text-primary)]">Story Collections</h2>
        ...
        <div className="bg-[var(--bg-secondary)] rounded-lg p-4 mb-4 border border-[var(--border-default)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <Input
              ...
              placeholder="e.g., Grandma's Stories"
              required
            />
            <Input
      ...
      <div className="max-h-[240px] overflow-y-auto pr-1 scrollbar-thin">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
          {sortedCollections.map((collection) => {
            ...
            const storyCount = collectionStoryCounts[collection.id] || 0;
            const coverImage = collection.cover_image_url || collectionCoverImages[collection.id];
